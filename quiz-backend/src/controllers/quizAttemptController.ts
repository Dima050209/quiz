...
import { createAttempt, getAttempt, updateAttempt as updAttempt } from "../services/quizAttemptService";
...
> = async (req, res) => {
  ...
  try {
    const attempt = await getAttempt({student_id: user.id, quiz_id: numQuizId});
    if(!attempt) {
    ...
    const updatedAttempt = await updAttempt({student_id: user.id, quiz_id: numQuizId, status});
...
> = async (req, res) => {
  ...
  try {
    ...
    const updatedAttempt = await updAttempt({...attemptUpd, student_id: user.id, quiz_id: numQuizId});
